...
from model.id_utils import (AttentionLayer, PositionEmbeddingSine,
...
class ISDHead(nn.Module):
    ...
    def forward(self, feature_map: torch.Tensor, idrs: torch.Tensor):
        ...
        # print(type(feature_map.clone()))
        # print(feature_map.clone().shape)
        for i in range(self.depth):
...
class AFP(nn.Module):
    ...
    def forward(
        self, feature_maps: Tuple[torch.Tensor, ...], idrs: Tuple[torch.Tensor, ...]
    ) -> Tuple[torch.Tensor, ...]:

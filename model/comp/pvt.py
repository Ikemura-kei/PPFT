...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 sr_ratios=[8, 4, 2, 1], num_stages=4, pretrained=None, pre_res=True, layer0=False):
        super().__init__()
        self.depths = depths
        self.num_stages = num_stages
        print("Use pretrained resnet:", pre_res)
        net = get_resnet34(pretrained=pre_res)
        # print(net)
        # exit()
        self.layer0=layer0
        if self.layer0:
            setattr(self, "embed_layer0", net.conv1)
    ...
    def init_weights(self, pretrained=None):
        if isinstance(pretrained, str):
            ...
            print(f"===pretrained weight loaded from {pretrained}===")
...
class PVT(PyramidVisionTransformer):
    def __init__(self, in_chans, patch_size=4, **kwargs):
        super(PVT, self).__init__(
            ...
            sr_ratios=[8, 4, 2, 1], drop_rate=0.0, drop_path_rate=0.1, pretrained=kwargs['pretrained'], pre_res=kwargs['pre_res'], layer0=kwargs['layer0'])

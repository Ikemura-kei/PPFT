...
class TransUnet(nn.Module):
    def __init__(self, args):
        ...
        self.n_channels = args.trans_n_channels
        self.n_classes = args.trans_n_classes
        self.bilinear = args.trans_bilinear
        self.skip_res = args.skip_res
        self.dim = args.trans_dim
        
        self.inc = DoubleConv(self.n_channels, 64)
        self.down1 = Down(self.dim, self.dim * 2, norm=args.norm)
        self.down2 = Down(self.dim * 2, self.dim * 4, norm=args.norm)
        self.down3 = Down(self.dim * 4, self.dim * 8, norm=args.norm)
        factor = 2 if self.bilinear else 1
        self.down4 = Down(self.dim * 8, self.dim * 16 // factor, norm=args.norm)
        ...
        self.outc = OutConv(self.dim, args.n_classes)
        self.resblock_layers = nn.ModuleList([])
        
        # Missing positinal encoding
        for i in range(args.residual_num):
            self.resblock_layers.append(Block(self.dim * 8, dropout=args.dropout))

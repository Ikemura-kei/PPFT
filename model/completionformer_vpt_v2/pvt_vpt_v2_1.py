...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        if use_prompt:
            ...
            self.mcp0 = MCPBlock(chs[0], chs[0], HID_DIM)

        for i in range(num_stages):
            if use_prompt:
                mcp = MCPBlock(chs[i+1], chs[i], HID_DIM)
                setattr(self, f"mcp{i + 1}", mcp)
                prompt_modifier = nn.Sequential(nn.Conv2d(chs[i], chs[i], kernel_size=3, stride=1, padding=1), \
...
class PVTVPTV2_1(PyramidVisionTransformer):
    def __init__(self, in_chans, patch_size=4, foundation=None, **kwargs):
        super(PVTVPTV2_1, self).__init__(

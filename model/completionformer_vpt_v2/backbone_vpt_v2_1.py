...
from .pvt_vpt_v2 import PVTVPTV2
from .pvt_vpt_v2_1 import PVTVPTV2_1
...
class BackboneVPTV2_1(nn.Module):
    def __init__(self, args, foundation, mode='rgbd'):
        super(BackboneVPTV2_1, self).__init__()
        self.args = args
        self.mode = mode
        ...
        if mode == 'rgbd':
            ...
            if self.args.pol_rep == 'grayscale-4':
                ...
                self.conv2_pol_for_rgb = conv_bn_relu(16, 128, kernel=3, stride=1, padding=1,
                                          bn=False)
                self.conv3_pol_for_rgb = conv_bn_relu(128, 48, kernel=3, stride=1, padding=1,
                                          bn=False)
        ...
        self.former = PVTVPTV2_1(in_chans=64, patch_size=2, pretrained='./model/completionformer_original/pretrained/pvt.pth', foundation=foundation.former)
    ...
    def forward(self, rgb=None, depth=None, pol=None):
        ...
        if self.mode == 'rgbd':
            ...
            fe1_pol_for_rgb = self.conv3_pol_for_rgb(\

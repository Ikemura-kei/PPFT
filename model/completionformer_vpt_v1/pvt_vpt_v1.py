...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        out_chs = [128, 320, 512, 512]

        if use_prompt:
            c1 = nn.Conv2d(64, 256, 5, 2, 1)
            nn.init.uniform_(c1.weight)
            c2 = nn.Conv2d(256, 128, 3, 1, 1)
            nn.init.uniform_(c2.weight)
            self.init_prompt_mod = nn.Sequential(c1, \
                                            c2, \
                                            nn.ReLU())

        for i in range(num_stages):
            if use_prompt:
                c1 = nn.Conv2d(in_chs[i], 256, 5, 2, 1)
                nn.init.uniform_(c1.weight)
                c2 = nn.Conv2d(256, out_chs[i], 3, 1, 1)
                nn.init.uniform_(c2.weight)
                prompt_mod = nn.Sequential(c1, \
                                            c2, \
                                            nn.ReLU())
                setattr(self, f"prompt_mod{i + 1}", prompt_mod)
    ...
    def forward_features(self, x, prompt=None):
        ...
        if prompt is not None:
            prompt = self.init_prompt_mod(x+prompt)
            x = x + prompt
        ...
        for i in range(self.num_stages):
            ...
            if prompt is not None:
                prompt_mod = getattr(self, f"prompt_mod{i + 1}")
                prompt = prompt_mod(x+prompt)
                x = x + prompt_mod
    ...
    def forward(self, x, rgb_prompt=None, dep_prompt=None):
...
class PVTVPTV1(PyramidVisionTransformer):
    def __init__(self, in_chans, patch_size=4, foundation=None, **kwargs):
        super(PVTVPTV1, self).__init__(

...
class PyramidVisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
        ...
        if use_prompt:
            ...
            #     num_resolutions=1,
            #     depth=2,
            #     pixel_dim=256, # input channel
            #     num_latents=32, # res of prompt
            #     latent_dim=128, # channel of prompt
            #     num_heads=1, # num of d vector in the projection space
            #     expansion=4, # num of hidden dimension of output MLP
            #     activation='gelu'
            # )
            ...
            #     activation='gelu'
            # )
            # self.prompt0 = nn.Parameter(torch.zeros([1, chs[0], heights[0], widths[0]]))
            # nn.init.uniform_(self.prompt0)

            # self.prompt_modifier0 = nn.Sequential(nn.Conv2d(48, chs[0], kernel_size=3, stride=1, padding=1), \
            ...
            # self.prompt_modifier0 = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
            HID_DIM = 256

            self.mcp0 = MCPBlockV2(chs[0], chs[0], HID_DIM)

        for i in range(num_stages):
            if use_prompt:
                # prompt = nn.Parameter(torch.zeros([1, chs[i+1]//2, heights[i+1]//2, widths[i+1]//2]))
                # nn.init.uniform_(prompt)
                # afp = AFP(
                #     num_resolutions=1,
                #     depth=1,
                #     pixel_dim=chs[i+1], # input channel
                #     num_latents=heights[i+1]//2+widths[i+1]//2, # res of prompt
                #     latent_dim=chs[i+1]//2, # channel of prompt
                #     num_heads=1, # num of d vector in the projection space
                #     expansion=2, # num of hidden dimension of output MLP
                #     activation='gelu'
                # )
                ...
                #     activation='gelu'
                # )
                # setattr(self, f"prompt{i + 1}", prompt)
                # setattr(self, f"afp{i+1}", afp)
                # setattr(self, f"isd{i+1}", isd)
                # prompt_modifier = PatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)),
                #                      patch_size= patch_size if i == 0 else 2,
                #                      in_chans=in_chans if i == 0 else embed_dims[i - 1],
                #                      embed_dim=embed_dims[i])

                mcp = MCPBlockV2(chs[i+1], chs[i+1], HID_DIM)
                setattr(self, f"mcp{i + 1}", mcp)
                # prompt_modifier = nn.Sequential(nn.Conv2d(chs[i], chs[i+1], kernel_size=3, stride=1, padding=1), \
                #                                 nn.ReLU(), nn.MaxPool2d(kernel_size=3, stride=2, padding=(1 if i != (num_stages-1) else 0)))
                ...
                # prompt_modifier = nn.MaxPool2d(kernel_size=2, stride=2, padding=(1 if i != (num_stages-1) else 0))
                # prompt_modifier = nn.MaxPool2d(kernel_size=2, stride=2, padding=0)
    ...
    def forward_features(self, x, prompt):
        ...
        if self.use_prompt:
            # x = x + self.prompt0.expand(B, -1, -1, -1)
            # x_idrs = self.afp0(x, self.prompt0.expand(B, -1, -1, -1))
            # x = self.isd0(x, x_idrs)

            prev_prompt = self.mcp0(x, self.prompt_modifier0(prompt))
            x = x + prev_prompt
        ...
        for i in range(self.num_stages):
            ...
            if self.use_prompt:
                # prompt = getattr(self, f"prompt{i + 1}")
                # afp = getattr(self, f"afp{i + 1}")
                # isd = getattr(self, f"isd{i + 1}")
                
                # id: isd(decoder_feat, afp(ori, prompt))
                # ours: isd(x_ori, afp(x_ori, prompt))
                # torch.Size([14, 64, 52, 68])
                # x_idrs = afp([x], [rearrange(prompt.expand(B, -1, -1, -1), "b c h w -> b (h w) c")])
                # x = isd([x], x_idrs)[0]
                # print(x.shape)
                # print(prev_prompt.shape)
                # print(getattr(self, 'prompt_modifier{}'.format(i+1))(prev_prompt).shape)
                # print('------')
                prev_prompt = getattr(self, f"mcp{i + 1}")(x, getattr(self, 'prompt_modifier{}'.format(i+1))(prev_prompt))
                # prev_prompt = getattr(self, f"mcp{i + 1}")(x, getattr(self, 'prompt_modifier{}'.format(i+1))(prev_prompt))
                x = x + prev_prompt

            # print('after', type(x))
            # print("--> Shape after stage {} is {}".format(i, x.shape))
            outs.append(x)

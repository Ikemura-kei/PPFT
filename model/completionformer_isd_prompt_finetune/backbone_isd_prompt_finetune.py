...
class BackboneISDPromptFinetune(nn.Module):
    def __init__(self, args, foundation, mode='rgbd'):
        ...
        if mode == 'rgbd':
            ...
            #     num_resolutions=1,
            #     depth=2,
            #     pixel_dim=48, # input channel
            #     num_latents=208*272, # res of prompt, h*w
            #     latent_dim=48, # channel of prompt
    ...
    def forward(self, rgb=None, depth=None, pol=None):
        ...
        if self.mode == 'rgbd':
            ...
            # fe1_pol_for_rgb_ori = self.conv4_pol_for_rgb(\
            #                         self.conv3_pol_for_rgb(\
            #                         self.conv2_pol_for_rgb(\
            #                         self.conv1_pol_for_rgb(pol))))
            # the prompt need to be in shape of [b, h*w, c]
            # torch.Size([14, 48, 208, 272])
            # torch.Size([14, 48, 208, 272])
            if self.args.pol_rep == 'grayscale-4':
